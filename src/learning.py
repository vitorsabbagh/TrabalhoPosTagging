...
def register_trans(tim1, ti):
    recover_tim1 = trans.get(tim1, {})
    recover_ti = recover_tim1.get(ti, 0) + 1
    recover_tim1[ti] = recover_ti
    trans[tim1] = recover_tim1


def register_emission(t, w):
    recover_t = emissions.get(t, {})
    recover_w = recover_t.get(w, 0) + 1
    recover_t[w] = recover_w
    emissions[t] = recover_t
    pass


def register_apriori(t):
    apriori[t] = apriori.get(t, 0) + 1
...
def separate_and_learn(filename):
    last_tag = "."
...
if __name__ == "__main__":
    ...
    separate_and_learn(r"./data/corpus100.txt")
    ...
    with open('./hmm_testing_data.json', 'w') as file:
