...
with open('hmm_testing_data.json') as json_file:
    json_file = json.load(json_file)
    testing_data = json_file['testing_data']

with open('hmm_learning_sufixes.json') as json_file:
    json_file = json.load(json_file)
...
def decode_ten_fold(ten_fold_index):
    ...
    for i in range(0, N):
        ...
        for j in range(0, len(wordset)):
            ...
            if not tag in emissions[str(ten_fold_index)]:
                matrix_B[i][j] = 0
            elif not word in emissions[str(ten_fold_index)][tag]:
                # if word in unknown_words:
                #     matrix_B[i][j] = sufixes[str(
                #         ten_fold_index)][tag][word[:-4]] / apriori_dict[str(ten_fold_index)][tag]
                # else:
                matrix_B[i][j] = 0
            else:
                matrix_B[i][j] = emissions[str(
                    ten_fold_index)][tag][word] / apriori_dict[str(ten_fold_index)][tag]
                # if word == 'nucléico' and ten_fold_index == 1:
                #     p('nucléico ' + tag + ' ')
                #     p(emissions[str(ten_fold_index)][tag][word])
                #     p(apriori_dict[str(ten_fold_index)][tag])
        #     p(matrix_B[i][j], end='  ')
        # p()
    ...
    for sentence in testing_data[str(ten_fold_index)]:
        ...
        tags_decoded = decoder.Decode(text_index_list)
...
for tag, value in accuracy.items():
    ...
    if 'correct' in value.keys():
        sum_correct += value['correct']
    if 'total' in value.keys():
...
with open('./hmm_accuracy.json', mode='w', encoding="utf8") as file:
